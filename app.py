...
if view == "National Overview":
    st.write("#### **Occupancy**")
    st.plotly_chart(fig)
    st.write(f"##### **Top 5 ICB With the Largest Absolute Occupancy Increase in {formatted_date} from the Month Prior**")
    st.table(functions.calculate_topn(vw_data, selected_date[0], selected_date[1], 1, 'Occupancy', 5))
    st.write("\n")
    st.write("#### **Capacity**")
    st.plotly_chart(fig1)
    st.write("\n")
    st.write(f"##### **Top 5 ICB With the Largest Absolute Capacity Increase in {formatted_date} from the Month Prior**")
    st.table(functions.calculate_topn(vw_data, selected_date[0], selected_date[1], 1, 'Capacity', 5))
    st.write("\n")
    st.write(f"##### **Top 5 ICB With the Largest Absolute Capacity Increase in {formatted_date} from 6 Months Prior**")
